...
const getVentas = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ message: `Ocurrio un error al obtener las ventas: ${err}` });
  }
};
...
const getVenta = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ message: `Ocurrio un error al obtener la venta: ${err}` });
  }
};
...
const postVenta = async (req, res) => {
  ...
  try {
    ...
    return res.status(200).json({ message: "Venta creada con éxito" });
  } catch (err) {
    return res
      .status(500)
      .json({ message: `Ocurrio un error al crear la venta: ${err}` });
  } finally {
    connection.release();
  }
};
...
const patchVenta = async (req, res) => {
  ...
  try {
    ...
    return res.status(200).json(updatedVentaResult[0]);
  } catch (err) {
    await connection.rollback();
    return res
      .status(500)
      .json({ message: `Ocurrio un error al actualizar la venta: ${err}` });
  } finally {
    connection.release();
  }
};
